...
# FILES_TO_LINK := src/systick_context_switcher.c
...
FILES_TO_LINK += $(SYSTEM_LIBRARY_SRC_DIR)/stm32f10x_dbgmcu.c
...
FILES_TO_LINK += bin/libtasks.a

# Build context switcher using systick
systick:
	@$(CC) $(CFLAGS) $(STARTUP_DIR)/startup_stm32f10x_md.s -o bin/contextSwitcher.elf src/systick_context_switcher.c $(FILES_TO_LINK) 
# Build context switcher using pendsv
pendsv:
	@$(CC) $(CFLAGS) $(STARTUP_DIR)/startup_stm32f10x_md.s -o bin/contextSwitcher.elf src/pendsv_context_switcher.c $(FILES_TO_LINK) 
# Program .elf into Crazyflie flash memory via the busblaster
flash:
	@openocd -d0 -f interface/busblaster.cfg -f target/stm32f1x.cfg -c init -c targets -c "reset halt" -c "flash write_image erase bin/contextSwitcher.elf" -c "verify_image bin/contextSwitcher.elf" -c "reset run" -c shutdown
# Halt program execution
halt:
	@openocd -f interface/busblaster.cfg -f target/stm32f1x.cfg -c 'init; targets; reset halt;' -c shutdown
...
debug:
	...
	@arm-none-eabi-gdb bin/contextSwitcher.elf -ex 'target remote:3333'
	@killall openocd

# Remove all files generated by target 'all'
clean:
	@rm bin/contextSwitcher.elf	
